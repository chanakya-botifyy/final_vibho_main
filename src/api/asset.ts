...
// Unassign asset
export const unassignAsset = async (id: string) => {
  try {
    const token = localStorage.getItem('token');
    if (!token) throw new Error('Not authenticated');

    const response = await axios.post(
      `${API_URL}/assets/${id}/unassign`,
      {},
      {
        headers: {
          Authorization: `Bearer ${token}`
...
export const addMaintenanceRecord = async (id: string, maintenanceData: any) => {
  try {
    const token = localStorage.getItem('token');
    if (!token) throw new Error('Not authenticated');

    const response = await axios.post(
      `${API_URL}/assets/${id}/maintenance`,
      maintenanceData,
      {
        headers: {
          Authorization: `Bearer ${token}`
